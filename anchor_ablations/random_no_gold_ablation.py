...
def generate_random_alignment_matrix(
    ...
    """Generate alignment matrix with completely random mappings (no gold mappings)."""
    random.seed(seed)
    ...
    for tid in tqdm(tids, desc="Generating random alignment"):
        # Count gold mappings but don't use them
        if tid in t2l_supl:
            gold_count += 1
        
        # Random mapping for ALL tokens (including ones that would have gold mappings)
        if tid not in ids1:
    ...
    print(f"  Tokens that would have had gold mappings: {gold_count} ({100*gold_count/total:.1f}%)")
    print(f"  Random assignments: {random_count} ({100*random_count/total:.1f}%)")
    print(f"  Note: ALL tokens are randomly mapped (no gold mappings used)!")
...
if __name__ == '__main__':
    ...
    parser.add_argument("-o", "--output-dir", type=str, default="./random_no_gold_evaluations")
    parser.add_argument("--output-name", type=str, default="alignment_matrix_random_no_gold.json",
                       help="Output filename (default: alignment_matrix_random_no_gold.json)")
    ...
    # gold src to tgt id transition json dict (loaded for statistics only)
    ...
    print(f"\n{'='*60}")
    print(f"Generating random alignment matrix (NO GOLD MAPPINGS)")
    ...
    print(f"\n{'='*60}")
    print(f"Random alignment matrix (no gold) generated")
